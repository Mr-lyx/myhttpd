...
private:
    ...
    int max_queue_number;//最大队列数
    pthread_t *threads;  // 线程数组
    queue<T *> task_queue;  // 任务队列
    MutexLocker queue_mutex_locker;  // 任务队列的互斥锁
    Cond queue_cond_locker;  // 任务队列为空的条件变量
    Cond queue_not_full;//任务队列是否满了
    bool m_stop;  // 是否结束线程
public:
...
template< typename T >
ThreadPool<T>::ThreadPool( int thread_num ) :thread_number(thread_num), max_queue_number(1024),
...
bool ThreadPool<T>::append( T *task ) {
    ...
    if(task_queue.size()>=max_queue_number){
    	queue_not_full.wait();
    }
